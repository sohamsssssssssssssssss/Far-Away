...
export async function generateReport(input: GenerateReportInput): Promise<string> {
  ...
  if (!content) {
    throw new Error('Ollama returned an empty report')
  }
...
async function resolveOllamaModel() {
  ...
  if (!response.ok) {
    throw new Error(`Ollama model list responded with ${response.status}`)
  }

  const data = (await response.json()) as { models?: Array<{ name?: string }> }
  const models = data.models?.map((model) => model.name).filter((model): model is string => Boolean(model)) ?? []
  const preferred = models.find((model) => model === 'llama3.2' || model.startsWith('llama3.2:'))

  return preferred ?? models[0] ?? 'llama3.2'
}
