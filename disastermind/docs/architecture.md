...
- **Operator console** — a browser UI lives under
  [`clients/operator-console/`](../clients/operator-console/) and talks to the
  dashboard API. A single-file vanilla-JS dashboard also ships at
  `disastermind/api/static/index.html` (served at `/`).
- **Field app** — a device-facing client lives under
  [`clients/field-app/`](../clients/field-app/); its wire contracts are in
  `disastermind/fieldapp/contracts.py`.
