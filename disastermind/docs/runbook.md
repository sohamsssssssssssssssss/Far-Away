...
**Operator console** — the richer React/Vite UI (map + charts + escalation queue):

```bash
cd clients/operator-console && npm install && npm run dev   # http://localhost:5173
```

It proxies `/health /topics /incidents /escalations /ws` to `:8000` by default
(override with `VITE_PROXY_TARGET`). See `clients/operator-console/README.md`.
